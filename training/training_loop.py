"""Drifting-model generator training loop.

Port of the JAX `train.py:train_gen` into the EDM2-style infrastructure of this
repo. The logging / status / snapshot / checkpoint / metric machinery is kept
intact; the per-step logic is the drift training step:

  1. Push freshly loaded reals into per-class (positive) and global
     (unconditional) memory-bank queues (Appendix A.8).
  2. Sample a batch of class labels; draw positives and unconditional
     negatives from the queues.
  3. Sample a CFG scale per label; generate `gen_per_label` samples per label.
  4. Compute the multi-feature drift loss and update the generator.

The drift loss is computed in the feature space of a frozen, pretrained
ResNet-MAE (Sec. 3.4), loaded from a snapshot pickle (``mae_pkl``).
"""
...
def training_loop(
    ...
    loss_microbatch_labels=None,   # Split the Nc labels into chunks of this size and accumulate grads. None/0 => no split (single full-batch call).

    old_bank_size=None,            # Per-class queue size for cached past-generated negatives. None/<=0 => disabled.
    old_per_sample=None,           # Cached generated negatives drawn per label. None/<=0 => disabled.
):
    ...
    assert num_classes > 0, 'Drifting models are class-conditional; use a labeled dataset.'
    ...
    if dist.get_rank() == 0:
        ...
        wandb_run = wandb.init(project='drifting', name=os.path.basename(run_dir),
    ...
    # Optional per-class queue of past-generated samples, reused as extra (frozen,
    # weight-1) negatives. Disabled when size/draw count is unset or non-positive.
    old_enabled = (old_bank_size is not None and old_bank_size > 0 and
                   old_per_sample is not None and old_per_sample > 0)
    bank_old = ArrayMemoryBank(num_classes=num_classes, max_size=old_bank_size) if old_enabled else None
    if old_enabled:
        dist.print0(f'Old-generated negative bank enabled: per-class size={old_bank_size}, draw={old_per_sample}/label.')
    ...
    while True:
        ...
        loss_accum = 0.0
        cfg_accum = 0.0
        # Use cached past-generated negatives once the bank holds samples. Sampling
        # reflects the bank as of step start; this step's fresh samples are pushed
        # only after the loop, so they never appear among their own negatives.
        use_old = old_enabled and bank_old.ready
        new_gen_images, new_gen_labels = [], []
        for i in range(0, Nc, mb):
            sl = slice(i, min(i + mb, Nc))
            chunk_w = sel_labels[sl].shape[0] / Nc
            is_last = (i + mb >= Nc)
            old_neg = bank_old.sample(sel[sl], old_per_sample).to(device).float() if use_old else None
        ...
        # Push this step's freshly generated samples into the old-generated bank.
        if old_enabled and new_gen_images:
