"""Build a config and launch drifting-model (generator) training.

Mirrors the two-preset layout of the original repo: `dataset_presets` holds
everything intrinsic to the data (generator architecture, monitoring sampler,
LR schedule family, EMA profiles); `config_presets` describes a particular run
(sample-allocation, CFG, optimization budget). The generator is trained with
the drift loss in the feature space of a pretrained ResNet-MAE (`--mae-pkl`),
which must be trained first via `train_mae.py`.
"""
...
_common_cifar = dict(
    ...
    cfg_min=1.0,
    cfg_max=4.0,
    neg_cfg_pw=3.0,       # p(alpha) ~ alpha^-3 (ablation default).
    no_cfg_frac=0.0,
...
def setup_training_config(preset='drift-cifar10', **opts):
    ...
    try:
        ...
        if not dataset_obj.has_labels:
            raise click.ClickException('Drifting models are class-conditional; dataset has no labels')
        del dataset_obj
    ...
    c.loss_kwargs = dnnlib.EasyDict(
        ...
        cfg_preserve_old=bool(opts.cfg_preserve_old),
    )
    ...
    c.old_bank_size = opts.old_bank_size
    c.old_per_sample = opts.old_per_sample
...
@click.option('--preset',           help='Configuration preset', metavar='STR', type=str, default='drift-cifar10', show_default=True)
...
@click.option('--gen-per-label',    help='Generated samples per label (Nneg)', metavar='INT', type=int, default=None)
@click.option('--pos-per-sample',   help='Positive samples per label (Npos)', metavar='INT', type=int, default=None)
@click.option('--neg-per-sample',   help='Unconditional negatives per label', metavar='INT', type=int, default=None)
...
@click.option('--old-bank-size',    help='Per-class queue size of cached generated negatives; <=0/unset disables', metavar='INT', type=int, default=None)
